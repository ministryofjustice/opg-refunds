resource "aws_rds_cluster" "caseworker" {
  ...
  vpc_security_group_ids = [aws_security_group.caseworker_rds_cluster.id]
  ...
  tags                            = merge(local.default_tags, local.caseworker_component_tag, map("scheduled_shutdown", "true"))
  lifecycle {
