...
resource "aws_ecs_service" "caseworker_front" {
  ...
  depends_on = [aws_rds_instance.applications, aws_lb.caseworker_front, aws_iam_role.caseworker_front_task_role, aws_iam_role.execution_role]
  ...
  caseworker_front_web = <<EOF
  {
    ...
    "logConfiguration": {
        ...
        "options": {
            ...
            "awslogs-stream-prefix": "caseworker-front-web.lpa-refunds"
        }
    },
    "environment": [
  ...
  caseworker_front_app = <<EOF
  {
    ...
    "logConfiguration": {
        ...
        "options": {
            "awslogs-group": "${data.aws_cloudwatch_log_group.lpa_refunds.name}",
            "awslogs-region": "eu-west-1",
            "awslogs-stream-prefix": "caseworker-front-app.lpa-refunds"
        }
    },
    "secrets": [
