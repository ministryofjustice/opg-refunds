...
resource "aws_cloudtrail" "cloudtrail" {
  name                          = "lpa_refunds_cloudtrail_${local.account_name}"
...
resource "aws_s3_bucket" "cloudtrail_logs" {
  bucket        = "lpa-refunds-cloudtrail-${local.account_name}"
  force_destroy = true
  acl           = "private"
...
data "aws_iam_policy_document" "cloudtrail_bucket_policy" {
  statement {
    ...
    resources = ["arn:aws:s3:::lpa-refunds-cloudtrail-${local.account_name}"]
  ...
  statement {
    ...
    resources = ["arn:aws:s3:::lpa-refunds-cloudtrail-${local.account_name}/prefix/AWSLogs/${data.aws_caller_identity.current.account_id}/*"]
