...
PGPASSWORD=$(aws secretsmanager get-secret-value --secret-id $ENV_NAME/postgres_password | jq -r .'SecretString') pg_restore --data-only --exit-on-error --table=finance --table=meris --table=sirius -h $CASES_DB_ENDPOINT -U root /mnt/sql/cases.sql

PGPASSWORD=$(aws secretsmanager get-secret-value --secret-id $ENV_NAME/opg_refunds_db_cases_migration_password | jq -r .'SecretString') pg_restore --data-only --exit-on-error --table=claim --table=doctrine_migration_versions --table=duplicate_claims --table=note --table=payment --table=poa --table=report --table=user --table=verification -h $CASES_DB_ENDPOINT -U cases_migration /mnt/sql/cases.sql
