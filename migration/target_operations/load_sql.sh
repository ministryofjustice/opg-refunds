...
# load
PGPASSWORD=$(aws secretsmanager get-secret-value --secret-id $ENV_NAME/postgres_password | jq -r .'SecretString') pg_restore --data-only --exit-on-error --table=finance -h $CASES_DB_ENDPOINT -U root --dbname=cases --verbose /mnt/sql/cases.tar
PGPASSWORD=$(aws secretsmanager get-secret-value --secret-id $ENV_NAME/postgres_password | jq -r .'SecretString') pg_restore --data-only --exit-on-error --table=meris  -h $CASES_DB_ENDPOINT -U root --dbname=cases --verbose /mnt/sql/cases.tar
PGPASSWORD=$(aws secretsmanager get-secret-value --secret-id $ENV_NAME/postgres_password | jq -r .'SecretString') pg_restore --data-only --exit-on-error --table=sirius -h $CASES_DB_ENDPOINT -U root --dbname=cases --verbose /mnt/sql/cases.tar
